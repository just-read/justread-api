...
const modifyBook = async (
  ...
  try {
    ...
    const {
      body: { bookId, title, isbn, description, year, authors },
    } = req;
    ...
    const existingBook = await getRepository(Book).findOne(bookId);

    if (!existingBook) {
      throw new NotFoundError('책 정보가 존재하지 않습니다.');
    }

    const modifiedBook = await getRepository(Book).update(bookId, {
      title,
      isbn,
      description,
      year,
